...
(function () {
  ...
  // navigated away (bfcache) — including the mid-fade "is-zooming" state
  // and the cloned zoom image left over from the transition. Clean both
  // up so Back always shows a normal, fully visible page.
  window.addEventListener('pageshow', function (e) {
    if (e.persisted) {
      document.body.classList.remove('is-zooming');
      var leftoverClones = document.querySelectorAll('.zoom-clone');
      for (var i = 0; i < leftoverClones.length; i++) {
        leftoverClones[i].remove();
      }
    }
  });
})();
